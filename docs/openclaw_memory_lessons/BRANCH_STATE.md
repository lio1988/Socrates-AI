...
- Goal 2 (machine-readable lesson store), Goal 7 (prompt registry),
  ...
  Goal 10 (local LLM provider), Goal 11 runtime (Shadow Apprentice Mode —
  the trace markers and identity evidence exist; the shadow runner itself
  does not), Goal 12 (UI panel), Goal 13 (fine-tuning dataset preparation
  beyond the Phase 22 corpus).
