...
description: Surface routing decisions on every response with a single opt-in header
seoTitle: 'Router Metadata | OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Router Metadata - Inspect Routing Decisions on Every Response'
og:description: 'Opt in to OpenRouter routing metadata to see which provider was selected, fallback attempts, and pipeline stages applied to your request.'
og:image: https://openrouter.ai/dynamic-og?title=Router%20Metadata&description=Inspect%20Routing%20Decisions%20on%20Every%20Response
og:image:width: 1200
og:image:height: 630
