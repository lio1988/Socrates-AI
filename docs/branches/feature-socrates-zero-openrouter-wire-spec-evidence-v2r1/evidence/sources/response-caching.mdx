...
description: Cache responses for identical API requests to save time and money
seoTitle: 'Response Caching | Cache API Responses with OpenRouter'
og:site_name: OpenRouter Documentation
og:title: 'Response Caching - Cache Identical API Responses'
og:description: "Cache identical LLM requests with OpenRouter's response caching feature. Reduce costs and latency for agent workflows, unit testing, and repeated context processing."
og:image: https://openrouter.ai/dynamic-og?title=Response%20Caching&description=Cache%20responses%20for%20identical%20API%20requests%20to%20save%20time%20and%20money
og:image:width: 1200
og:image:height: 630
...
```http title="Response Headers (MISS)" lines
HTTP/2 200
X-OpenRouter-Cache-Status: MISS
X-OpenRouter-Cache-TTL: 300
```

```json title="Response Body (MISS)" lines
{
  "id": "gen-abc123",
  "model": "google/gemini-2.5-flash",
  "choices": ["..."],
...
```http title="Response Headers (HIT)" lines
HTTP/2 200
...
```json title="Response Body (HIT)" lines
{
  "id": "gen-def456",
  "created": 1746000012,
  "model": "google/gemini-2.5-flash",
