...
Pre-freeze all scientific semantics, execute exactly one authoritative 23-case
...
1. [done] Verify the Phase 8R decision branch, predecessor artifact, historical
   hashes, protected files, and unchanged runtime boundary.
2. [done] Create `feature/socrates-zero-canonical-successor-parity-v2` and add
   the 34-file core/predecessor lock without changing runtime semantics.
3. [done] Define the exact 44-guard order, 16-code taxonomy,
   `FIRST_CANONICAL_GUARD_WINS`, advisory diagnostics, 15-field vector, and the
   eleven orthogonal plus seven precedence probe contracts.
4. [done] Define the reference-only five-case links, 23-member case set and
   composite corpus, additive harness/evidence/metrics/threshold/artifact/replay
   contracts, strict validity gate, ground-truth firewall, isolation controls,
   and write-once APIs in the current worktree.
5. [done] Write the dedicated PRE-RESULT experiment specification and synchronize
   branch memory, plan, present state, and artifact-directory policy.
6. [done] Complete independent pre-result code/test review and all
...
Stop if any orthogonal probe changes a second undeclared independent input, any
target guard is not reachable with every earlier guard preserved, any observed
literal/vector/stage/submission evidence differs, expected truth enters a
runtime closure, any advisory field requires later validation, any frozen core
or historical/predecessor lock differs, any runtime semantic change is needed,
or any live/provider/model/tool call occurs.

## Stop conditions after the aggregate

The first artifact is immutable. On any threshold failure, preserve it as
