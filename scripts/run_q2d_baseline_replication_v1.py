"""Q2d replication: five independent baseline samples per model.
...
def _question() -> str:
    ...
    if q2.QUESTION_SHA256_V1 != QUESTION_SHA256_V1:
        raise SystemExit("Q2d question text drifted from its pinned digest")
...
def main() -> int:
    ...
    print("=== Q2d baseline replication: 5 samples x 3 models ===")
    for key in families:
        print(f"  {q1.FAMILIES_V1[key]['label']:18s} "
              f"{q1.FAMILIES_V1[key]['selector']:22s} "
              f"${q1._usd(bounds[key])} per call")

    run_id = f"q2d-baseline-replication-{int(time.time())}"
    ...
    out = q1.RUNS / "q2d_baseline_replication_v1.json"
    out.write_text(
        canonical_json(
            {
                "schema_version": "socrates-q2d-baseline-replication/v1",
                "run_id": run_id,
