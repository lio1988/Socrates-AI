...
Verifies the acceptance criteria mechanically:
  ...
  - the exact synthesis memory lessons selected by the council ledger reach
    the apprentice (same context key the CED wiring uses)
...
def _weak_council(*, lessons=None):
...
# --------------------------------------------------------------------------- #
# Lessons reach the apprentice (exact synthesis selection from council ledger)
# --------------------------------------------------------------------------- #

def test_lessons_reach_apprentice_context():
    lessons = load_stable_lessons()
    appr = MarkerProvider("shadow_appr", STRONG)
    runner = _runner(apprentice=appr, lessons=lessons)
    final, record = _shadow(
        runner,
...
def test_keywordless_question_still_replays_phase_matched_lessons():
    # Keyword-less does NOT mean no injection: the real CED retrieval is phase-
    # aware. The apprentice must replay the synthesis ledger rather than repeat
    # the old question-only audit mistake.
    lessons = load_stable_lessons()
    appr = MarkerProvider("shadow_appr", STRONG)
    runner = _runner(apprentice=appr, lessons=lessons)
    final, record = _shadow(
