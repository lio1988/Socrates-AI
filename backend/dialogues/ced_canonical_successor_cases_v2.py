...
FROZEN_CANONICAL_SUCCESSOR_PROBES_V2: Tuple[CanonicalSuccessorProbeV2, ...] = (
    ...
    _probe(probe_id=ORTHOGONAL_PROBE_IDS[5], probe_class=ProbeClass.ORTHOGONAL, stage=ProbeStage.APPLY, construction_rule="Rename only the active agent to phase8-recorded-agent-1-v2 while preserving its active provider seat and sorted rank; task-semantic identity is the intended changed relation, while full binding/configuration/lineage identities change dependently. Hold the accepted observation fixed.", literal_mutations=(_m("root.active_agent_id", _AGENT, _AGENT_RENAMED),), vector=_v(D, D, D, I, P, P, P, P, P, P, P, D, D, P, P), reason=SuccessorUnavailableReason.OBSERVATION_TASK_MISMATCH, guard="A9", primary_fields=("agent_id", "task_semantic_digest"), advisory_fields=("source_capsule_id", "source_configuration_digest", "source_execution_id"), structured_scan=True),
    _probe(probe_id=ORTHOGONAL_PROBE_IDS[6], probe_class=ProbeClass.ORTHOGONAL, stage=ProbeStage.APPLY, construction_rule="Change only the root question to What is knowledge? while retaining the accepted session ID and exact observation; root identity is the intended changed relation, and the CED task context digest itself remains equal while session/request bindings change.", literal_mutations=(_m("root.question", _BASE_QUESTION, _NEW_QUESTION),), vector=_v(I, D, D, D, P, P, P, P, P, P, P, P, D, P, P), reason=SuccessorUnavailableReason.ROOT_CONTEXT_MISMATCH, guard="A8", primary_fields=("request_semantic_digest", "source_session_semantic_id"), advisory_fields=("source_capsule_id", "source_execution_id", "task_semantic_digest"), structured_scan=True),
    _probe(probe_id=ORTHOGONAL_PROBE_IDS[7], probe_class=ProbeClass.ORTHOGONAL, stage=ProbeStage.APPLY, construction_rule="Change only the active agent's authoritative exact model from phase8-recorded-model/1 to phase8-recorded-model/1-v2-orthogonal while retaining the public display roster and narrow active provider seat; the two derived full ProviderBindingIdentity rows sharing that adapter change dependently.", literal_mutations=(_m("root.active_model_id", _MODEL, _MODEL_RENAMED),), vector=_v(D, D, D, D, P, P, P, P, P, P, I, D, D, P, P), reason=SuccessorUnavailableReason.OBSERVATION_MODEL_MISMATCH, guard="A11", primary_fields=("actual_model_id", "configured_model_id"), advisory_fields=("model_config_digest", "source_capsule_id", "source_configuration_digest", "source_execution_id", "task_model_config_digest"), structured_scan=True),
    _probe(probe_id=ORTHOGONAL_PROBE_IDS[8], probe_class=ProbeClass.ORTHOGONAL, stage=ProbeStage.APPLY, construction_rule="Change only provider_timeout_seconds from 30.0 to 31.0 and hold task-guard fields, provider, model, action, and observation fixed.", literal_mutations=(_m("runtime.provider_timeout_seconds", 30.0, 31.0),), vector=_v(D, D, D, P, P, P, P, P, P, P, P, I, D, P, P), reason=SuccessorUnavailableReason.OBSERVATION_CONFIG_MISMATCH, guard="A12", primary_fields=("source_configuration_digest",), advisory_fields=("source_capsule_id", "source_execution_id"), structured_scan=True),
