...
FROZEN_OPENROUTER_ROUTE_CONTROL_ORTHOGONAL_REQUEST_PROBES_V1: Tuple[
    ...
    _probe("orroutev1-or11-base-provider-selector", _K.ORTHOGONAL_REQUEST, "BASE_PROVIDER_SELECTOR", _G.G06_EXACT_SINGLETON_ENDPOINT, _F.BASE_PROVIDER_SELECTOR_FORBIDDEN, "Use the broad provider label instead of the endpoint selector.", (_mutation("request.body.provider.only", ["azure/swedencentral"], ["azure"], "endpoint_restriction"),), _orthogonal_request_vector("endpoint_restriction", body_bytes=_D, receipt_identity=_D)),
    ...
    _probe("orroutev1-or21-tools-enabled", _K.ORTHOGONAL_REQUEST, "TOOLS_ENABLED", _G.G12_TOOLS_DISABLED, _F.TOOLS_NOT_DISABLED, "Add one tool declaration.", (_mutation("request.body.tools", [], [{"name": "forbidden-tool"}], "tools"),), _orthogonal_request_vector("tools", body_bytes=_D, receipt_identity=_D)),
    ...
    _probe("orroutev1-or28-noncanonical-body", _K.ORTHOGONAL_REQUEST, "NONCANONICAL_BODY", _G.G15_CANONICAL_BYTES, _F.NONCANONICAL_BODY, "Change only JSON whitespace in the prepared body representation.", (_mutation("request.serialization.body_json", "{\"model\":\"openai/gpt-4.1-mini\"}", "{ \"model\":\"openai/gpt-4.1-mini\"}", "body_bytes"),), _orthogonal_request_vector("body_bytes", receipt_identity=_D)),
...
FROZEN_OPENROUTER_ROUTE_CONTROL_ORTHOGONAL_RESPONSE_PROBES_V1: Tuple[
    ...
    _probe("orroutev1-os06-wrong-actual-model", _K.ORTHOGONAL_RESPONSE, "WRONG_ACTUAL_MODEL", _G.G26_ACTUAL_MODEL_MATCH, _F.ACTUAL_MODEL_SUBSTITUTION, "Substitute the actual model with another exact model ID.", (_mutation("response.model", "openai/gpt-4.1-mini", "openai/gpt-4.1", "actual_model"),), _orthogonal_response_vector("actual_model", receipt_identity=_D)),
    _probe("orroutev1-os07-wrong-provider", _K.ORTHOGONAL_RESPONSE, "WRONG_PROVIDER", _G.G28_PROVIDER_COMPATIBILITY, _F.PROVIDER_SUBSTITUTION, "Substitute the broad provider attestation.", (_mutation("response.openrouter_metadata.endpoints.available[0].provider", "azure", "google", "provider"),), _orthogonal_response_vector("provider", receipt_identity=_D)),
    _probe("orroutev1-os08-actual-model-missing", _K.ORTHOGONAL_RESPONSE, "ACTUAL_MODEL_MISSING", _G.G25_ACTUAL_MODEL_PRESENCE, _F.ACTUAL_MODEL_MISSING, "Remove the actual model field.", (_mutation("response.model", "openai/gpt-4.1-mini", {"state": "ABSENT"}, "actual_model"),), _orthogonal_response_vector("actual_model", receipt_identity=_D)),
    _probe("orroutev1-os09-provider-missing", _K.ORTHOGONAL_RESPONSE, "PROVIDER_MISSING", _G.G27_PROVIDER_PRESENCE, _F.PROVIDER_MISSING, "Remove the documented broad provider attestation.", (_mutation("response.openrouter_metadata.endpoints.available[0].provider", "azure", {"state": "ABSENT"}, "provider"),), _orthogonal_response_vector("provider", receipt_identity=_D)),
    ...
    _probe("orroutev1-os11-several-attempts", _K.ORTHOGONAL_RESPONSE, "SEVERAL_ATTEMPTS", _G.G29_ATTEMPTS_CONSISTENCY, _F.MULTIPLE_ATTEMPTS_REPORTED, "Add a second record to the optional attempts list.", (_mutation("response.openrouter_metadata.attempts", [{"attempt": 1}], [{"attempt": 1}, {"attempt": 2}], "attempts_list"),), _orthogonal_response_vector("attempts_list", receipt_identity=_D)),
    ...
    _probe("orroutev1-os13-cache-hit-metadata-unavailable", _K.ORTHOGONAL_RESPONSE, "CACHE_HIT_METADATA_UNAVAILABLE", _G.G22_CACHE_METADATA_AVAILABILITY, _F.CACHE_AFFECTED_METADATA_UNAVAILABLE, "Mark the canned response as cache-affected with metadata unavailable.", (_mutation("response.cache_state", "MISS", "HIT_METADATA_UNAVAILABLE", "cache_state"),), _orthogonal_response_vector("cache_state", metadata_presence=_D, receipt_identity=_D)),
    ...
    _probe("orroutev1-os16-fallback-indicator", _K.ORTHOGONAL_RESPONSE, "FALLBACK_INDICATOR", _G.G30_FALLBACK_PIPELINE_ABSENCE, _F.FALLBACK_INDICATOR_PRESENT, "Add an explicit fallback-used indicator.", (_mutation("response.openrouter_metadata.fallback_used", {"state": "ABSENT"}, True, "fallback"),), _orthogonal_response_vector("fallback", receipt_identity=_D)),
    _probe("orroutev1-os17-alias-model-substitution", _K.ORTHOGONAL_RESPONSE, "ALIAS_MODEL_SUBSTITUTION", _G.G26_ACTUAL_MODEL_MATCH, _F.ACTUAL_MODEL_SUBSTITUTION, "Replace the actual model with an alias-like value.", (_mutation("response.model", "openai/gpt-4.1-mini", "openai/gpt-4.1-mini:latest", "actual_model"),), _orthogonal_response_vector("actual_model", receipt_identity=_D)),
    _probe("orroutev1-os18-provider-substitution", _K.ORTHOGONAL_RESPONSE, "PROVIDER_SUBSTITUTION", _G.G28_PROVIDER_COMPATIBILITY, _F.PROVIDER_SUBSTITUTION, "Replace the broad provider label with an alternate provider.", (_mutation("response.openrouter_metadata.endpoints.available[0].provider", "azure", "openai", "provider"),), _orthogonal_response_vector("provider", receipt_identity=_D)),
    _probe("orroutev1-os19-forbidden-pipeline-stage", _K.ORTHOGONAL_RESPONSE, "FORBIDDEN_PIPELINE_STAGE", _G.G30_FALLBACK_PIPELINE_ABSENCE, _F.FORBIDDEN_PIPELINE_STAGE, "Add a pipeline stage indicating alternate-route traversal.", (_mutation("response.openrouter_metadata.pipeline", [], [{"stage": "fallback"}], "fallback"),), _orthogonal_response_vector("fallback", receipt_identity=_D)),
    _probe("orroutev1-os20-cache-hit-with-metadata", _K.ORTHOGONAL_RESPONSE, "CACHE_HIT_WITH_METADATA", _G.G22_CACHE_METADATA_AVAILABILITY, _F.CACHE_HIT_RESPONSE_REJECTED, "Mark a metadata-bearing canned response as an explicit cache hit.", (_mutation("response.cache_state", "MISS", "HIT_WITH_METADATA", "cache_state"),), _orthogonal_response_vector("cache_state", receipt_identity=_D)),
)
...
FROZEN_OPENROUTER_ROUTE_CONTROL_PRECEDENCE_PROBES_V1: Tuple[
    ...
    _probe("orroutev1-p03-cache-header-missing-plus-metadata-missing", _K.PRECEDENCE, "CACHE_HEADER_MISSING_PLUS_METADATA_MISSING", _G.G14_CACHE_HEADER, _F.CACHE_HEADER_MISSING, "Missing cache-disable header prevents dispatch before missing metadata can be evaluated.", (_mutation("request.headers.X-OpenRouter-Cache", "false", {"state": "ABSENT"}, "cache_header"), _mutation("response.openrouter_metadata", {"state": "PRESENT"}, {"state": "ABSENT"}, "metadata_presence")), _vector(cache_header=_I, metadata_presence=_I, header_bytes=_D, receipt_identity=_D)),
    _probe("orroutev1-p04-metadata-missing-plus-model-mismatch", _K.PRECEDENCE, "METADATA_MISSING_PLUS_MODEL_MISMATCH", _G.G20_METADATA_PRESENCE, _F.ROUTER_METADATA_MISSING, "Missing metadata precedes response model mismatch.", (_mutation("response.openrouter_metadata", {"state": "PRESENT"}, {"state": "ABSENT"}, "metadata_presence"), _mutation("response.model", "openai/gpt-4.1-mini", "openai/gpt-4.1", "actual_model")), _vector(metadata_presence=_I, actual_model=_I, attempt=_D, provider=_D, receipt_identity=_D)),
    _probe("orroutev1-p05-multi-attempt-plus-model-substitution", _K.PRECEDENCE, "MULTI_ATTEMPT_PLUS_MODEL_SUBSTITUTION", _G.G24_ATTEMPT_ONE, _F.MULTI_ATTEMPT_ROUTING_OBSERVED, "Multi-attempt observation precedes actual-model substitution.", (_mutation("response.openrouter_metadata.attempt", 1, 2, "attempt"), _mutation("response.model", "openai/gpt-4.1-mini", "openai/gpt-4.1", "actual_model")), _vector(attempt=_I, actual_model=_I, receipt_identity=_D)),
    _probe("orroutev1-p06-malformed-metadata-plus-fake-endpoint", _K.PRECEDENCE, "MALFORMED_METADATA_PLUS_FAKE_ENDPOINT", _G.G21_METADATA_SCHEMA, _F.ROUTER_METADATA_MALFORMED, "Malformed metadata precedes unsupported exact-endpoint claim rejection.", (_mutation("response.openrouter_metadata", {"state": "PRESENT"}, "malformed", "metadata_presence"), _mutation("response.openrouter_metadata.endpoint_slug", {"state": "ABSENT"}, "azure/swedencentral", "endpoint_attestation_claim")), _vector(metadata_presence=_I, endpoint_attestation_claim=_I, attempt=_D, actual_model=_D, provider=_D, receipt_identity=_D)),
